...
def preprocess(self, img: PIL.Image) -> torch.Tensor:
    ...
    pad_value = tuple(self.config['pad_value'])
    img_mean = tuple(self.config['img_mean'])
...
def postprocess(self, pose_output: torch.Tensor, input_img: PIL.Image, visualize: bool = False):
    ...
    if visualize:
        ...
        for kpt_idx in range(18):  # 19th for bg
            total_keypoints_num += utils.extract_keypoints(heatmaps[:, :, kpt_idx], all_keypoints_by_type, total_keypoints_num)

        pose_entries, all_keypoints = utils.group_keypoints(all_keypoints_by_type, pafs, demo=True)
